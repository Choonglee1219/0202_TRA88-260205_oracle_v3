  ...
  implements OBC.Disposable, OBC.Eventable
{
  ...
  async updateColors() {
    ...
    const otherStyles: [string, OBC.ModelIdMap][] = [];
    let selectStyleData: [string, OBC.ModelIdMap] | null = null;

    // "select" 스타일과 다른 스타일들을 분리합니다.
    for (const [style, modelIdMap] of Object.entries(this.selection)) {
      if (style === this.config.selectName) {
        selectStyleData = [style, modelIdMap];
      } else {
        otherStyles.push([style, modelIdMap]);
      }
    }

    // "select"가 아닌 다른 스타일들을 먼저 적용합니다.
    for (const [style] of otherStyles) {
      const definition = this.styles.get(style);
      if (!definition) continue;
      // getMapWithoutSelection을 통해 "select"된 아이템에는 이 스타일이 적용되지 않도록 합니다.
      const map = this.getMapWithoutSelection(style);
      if (!map) continue;
      await fragments.highlight({ ...definition, customId: style }, map);
    }

    // "select" 스타일을 가장 마지막에 적용하여 최우선 순위를 갖도록 합니다.
    if (selectStyleData) {
      const [style, modelIdMap] = selectStyleData;
      const definition = this.styles.get(style);
      if (definition && !OBC.ModelIdMapUtils.isEmpty(modelIdMap)) {
        await fragments.highlight({ ...definition, customId: style }, modelIdMap);
      }
    }
