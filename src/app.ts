...
async function setupDatabase() {
  ...
  try {
    ...
    try {
      await connection.execute(ifcBcfSQL);
    } catch (error) {
      console.error("Error setting up ifc_bcf table:", error);
    }
    
    // 기존 테이블에 컬럼이 없는 경우를 대비한 자동 ALTER 처리 (에러 무시)
    try {
      await connection.execute(`ALTER TABLE "bcf" ADD "clash_data" CLOB`);
    } catch (error) {
      // ORA-01430: 열이 이미 추가되었습니다. (무시)
    }
  } catch (error) {
